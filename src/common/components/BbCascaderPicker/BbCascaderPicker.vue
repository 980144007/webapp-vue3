...
<template>
  <div class="bb-cascader-picker-container">
    <van-field v-if="type === 'input'" :modelValue="selectedObj.label" :name="name" :label="label" :required="required"
      readonly :is-link="!disabled && !readonly" :disabled="disabled" @click.stop="onShowChange"
      :placeholder="placeholder || `请选择${label}`" :rules="rules[0] ? rules : [{ required, message: `请选择${label}` }]">
    ...
    <van-popup v-model:show="showPicker" round position="bottom" teleport="body">
      <van-cascader :modelValue="modelValue" :title="`请选择${label}`" :options="options" @close="showPicker = false"
        @finish="onConfirm">
